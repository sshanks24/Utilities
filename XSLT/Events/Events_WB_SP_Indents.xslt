<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="HMI" select="document('XP_HMI.xml')/Root"/>
	<xsl:variable name="deviceFamily" select="document('Suite_Info.xml')//deviceFamily"/>
	<xsl:variable name="deviceType" select="document('Suite_Info.xml')//deviceType"/>
	<xsl:variable name="deviceName"><xsl:value-of select="$deviceFamily"/>/<xsl:value-of select="$deviceType"/></xsl:variable>
	<xsl:variable name="tabName" select="document('Suite_Info.xml')//tabName"/>
	<xsl:variable name="idPrefix" select="document('Suite_Info.xml')//idPrefix"/>
	<xsl:variable name="WB_Monitor" select="document('WB_Monitor.xml')/project_test_case"/>
	<xsl:variable name="SP_Monitor" select="document('SP_Monitor.xml')/project_test_case"/>
